...
plugin and the extension for Visual Studio Code and compatible editors add editor support for `.sdd` files.
...
- Use the SpecDD CLI for project setup, updates, checks, and Agent Skills deployment.
- Author or revise `.sdd` specs while preserving ownership, authority, and SpecDD language rules.
...
Use the extension for editor support while working with `.sdd` specification files in Visual Studio Code and compatible
editors. It provides semantic syntax highlighting, strict diagnostics, document symbols, folding ranges, completions,
hovers, document links for path and glob references, related-spec commands, workspace validation, and SpecDD file icons.

Source repository: [github.com/specdd/vscode-plugin](https://github.com/specdd/vscode-plugin)

Visual Studio Marketplace page: [marketplace.visualstudio.com/items?itemName=specdd.specdd](https://marketplace.visualstudio.com/items?itemName=specdd.specdd)

Open VSX page: [open-vsx.org/extension/specdd/specdd](https://open-vsx.org/extension/specdd/specdd)

Install from Visual Studio Code or a compatible editor:
...
Install from the command line in Visual Studio Code:

```bash
code --install-extension specdd.specdd
```

For compatible editors, use the editor's extension marketplace or replace `code` with that editor's launcher when it
supports VS Code-style extension installation.
